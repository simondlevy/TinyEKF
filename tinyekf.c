...
{    
    ...
    mulmat(tmp1, &ekf->Ft[0][0], &ekf->Pp[0][0], N, N, N);
    add(&ekf->Pp[0][0], &ekf->Q[0][0], N, N);

    // G_k = P_k H^T_k (H_k P_k H^T_k + R)^{-1}
    transpose(&ekf->H[0][0], &ekf->Ht[0][0], M, N);
    mulmat(&ekf->Pp[0][0], &ekf->Ht[0][0], tmp1, N, N, M);
    mulmat(&ekf->H[0][0], &ekf->Pp[0][0], tmp2, M, N, N);
    mulmat(tmp2, &ekf->Ht[0][0], tmp3, M, N, M);
    add(tmp3, &ekf->R[0][0], M, M);
    invert(tmp3, tmp4, tmp5, M);
    mulmat(tmp1, tmp4, &ekf->G[0][0], N, M, M);

    // \hat{x}_k = \hat{x_k} + G_k(z_k - h(\hat{x}_k
    sub(tmp1, ekf->hx, Z, M);
    mulvec(&ekf->G[0][0], tmp1, &ekf->x[0], N, M);

    // P_k = (I - G_k H_k) P_k
    mulmat(&ekf->G[0][0], &ekf->H[0][0], tmp1, N, M, N);
    negate(tmp1, N, N);
    mat_addeye(tmp1, N);
    mulmat(tmp1, &ekf->Pp[0][0], &ekf->P[0][0], N, N, N);
